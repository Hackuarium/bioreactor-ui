...
} from '../../pages/LocalDeviceDetails/Contexts';
    ...
    if (confStep[index] === 'Action') {
      ...
      if (actStep[index] === actions[6].label) {
        ...
        flagTemp.map((flag, indexFlag) => {
          if (indexFlag < 6) {
            if (flag === '1')
              tActiveFlag = [...tActiveFlag, flags[indexFlag].label];
          }
          return tActiveFlag;
        });
        ...
        tActiveFlag = [
          ...tActiveFlag,
          Number(`0b${step.slice(5, 16).join('')}`),
        ];
      }
...
const CardSteps = () => {
  ...
  // Calculate Table
  const [confStep, actStep, flagStep] = useMemo(
    () => calculateTable({ tableValues }),
    [tableValues],
  );
      ...
      <div className="m-2 p-2 flex flex-col justify-between items-center sm:items-start rounded-md bg-blue-gray-100 shadow-md">
        <div className="w-full py-1 flex flex-row justify-between items-start relative">
          <h3 className="text-sm font-medium text-neutral-700">
            {stepParameter.name || stepParameter.label}
          </h3>
